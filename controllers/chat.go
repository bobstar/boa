...
func (this *MainController) IM() {
	this.Data["host"] = this.Ctx.Request.Host
	this.TplName = "聊天室/聊天室.html"
}
...
func (c *connection) readPump() {
	...
	for {
		...
		case websocket.TextMessage:
			...
			h.broadcast <- []byte(time.Now().Format("15:04:05") + " " + c.username + ":" + string(message))
		}
	}
}
...
func (w *WSController) Get() {
	...
	c := &connection{send: make(chan []byte, 256), ws: ws, username: "权文博"}
	h.register <- c
	go c.writePump()
	c.readPump()
}
